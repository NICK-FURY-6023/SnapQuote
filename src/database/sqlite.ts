...
export async function getDb(): Promise<SQLite.SQLiteDatabase> {
  if (!db) {
    db = await SQLite.openDatabase({ name: 'snapquote-v2.db', location: 'default' });
    await initTables();
  }
  return db;
}
...
async function initTables(): Promise<void> {
  ...
  await d.executeSql('PRAGMA journal_mode = WAL;');
  await d.executeSql('PRAGMA foreign_keys = ON;');
  ...
  await d.executeSql('CREATE INDEX IF NOT EXISTS idx_quotations_user ON quotations(user_id);');
  await d.executeSql('CREATE INDEX IF NOT EXISTS idx_quotations_sync ON quotations(sync_status);');
  await d.executeSql('CREATE INDEX IF NOT EXISTS idx_items_quotation ON quotation_items(quotation_id);');
  await d.executeSql('CREATE INDEX IF NOT EXISTS idx_clients_user ON clients(user_id);');
  await d.executeSql('CREATE INDEX IF NOT EXISTS idx_clients_phone ON clients(phone);');
  await d.executeSql('CREATE INDEX IF NOT EXISTS idx_templates_user ON templates(user_id);');

  // Ensure default settings exist
  const [existing] = await d.executeSql(
    'SELECT COUNT(*) as count FROM settings WHERE id = ?',
    ['default'],
  );
  if (existing.rows.length === 0 || existing.rows.item(0).count === 0) {
    await d.executeSql(
...
export async function getAllQuotations(): Promise<Quotation[]> {
  const d = await getDb();
  const [results] = await d.executeSql('SELECT * FROM quotations ORDER BY created_at DESC');
...
export async function getQuotationById(id: string): Promise<Quotation | null> {
  const d = await getDb();
  const [results] = await d.executeSql('SELECT * FROM quotations WHERE id = ?', [id]);
...
export async function deleteQuotation(id: string): Promise<void> {
  const d = await getDb();
  await d.executeSql('DELETE FROM quotation_items WHERE quotation_id = ?', [id]);
  await d.executeSql('DELETE FROM quotations WHERE id = ?', [id]);
}
...
export async function saveQuotationItems(items: QuotationItem[]): Promise<void> {
  ...
  for (const item of items) {
    await d.executeSql(
      ...
      [item.id, item.quotation_id, item.item_no, item.item_name, item.quantity,
      item.unit, item.rate, item.discount_type, item.discount_value,
      item.discount_amount, item.tax_percent, item.tax_amount, item.total,
...
export async function deleteQuotationItems(quotationId: string): Promise<void> {
  const d = await getDb();
  await d.executeSql('DELETE FROM quotation_items WHERE quotation_id = ?', [quotationId]);
}
...
export async function getAllClients(): Promise<Client[]> {
  const d = await getDb();
  const [results] = await d.executeSql('SELECT * FROM clients ORDER BY name ASC');
  const rows: Client[] = [];
  for (let i = 0; i < results.rows.length; i++) {
    rows.push(results.rows.item(i));
  }
...
export async function getClientByPhone(phone: string): Promise<Client | null> {
  const d = await getDb();
  const [results] = await d.executeSql('SELECT * FROM clients WHERE phone = ?', [phone]);
  if (results.rows.length > 0) {
    return results.rows.item(0);
  }
  return null;
...
export async function getAllTemplates(): Promise<Template[]> {
  const d = await getDb();
  const [results] = await d.executeSql('SELECT * FROM templates ORDER BY usage_count DESC');
  const rows: Template[] = [];
  for (let i = 0; i < results.rows.length; i++) {
    rows.push(results.rows.item(i));
  }
...
export async function getTemplateWithItems(id: string): Promise<{ template: Template; items: TemplateItem[] } | null> {
  const d = await getDb();
  const [templateResults] = await d.executeSql('SELECT * FROM templates WHERE id = ?', [id]);
  if (templateResults.rows.length === 0) return null;
  const template = templateResults.rows.item(0);
  const [itemResults] = await d.executeSql(
...
export async function saveTemplate(template: Template, items: TemplateItem[]): Promise<void> {
  ...
  // Replace items
  await d.executeSql('DELETE FROM template_items WHERE template_id = ?', [template.id]);
  for (const item of items) {
    await d.executeSql(
...
export async function getSettings(): Promise<Settings | null> {
  const d = await getDb();
  const [results] = await d.executeSql('SELECT * FROM settings WHERE id = ?', ['default']);
  if (results.rows.length > 0) {
    return results.rows.item(0);
  }
  return null;
...
export async function saveSettings(s: Partial<Settings>): Promise<void> {
  ...
  const keys = Object.keys(s).filter((k) => k !== 'id' && k !== 'created_at' && k !== 'updated_at');
  if (keys.length === 0) return;
  const fields = keys.map((k) => `${k} = ?`).join(', ');
  const values = keys.map((k) => (s as any)[k] === undefined ? null : (s as any)[k]);
  await d.executeSql(
